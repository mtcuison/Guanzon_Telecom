...
public class AppData extends SQLiteOpenHelper {
    ...
    private String getEmployeeInfo(String sFieldNm){
        ...
        try{
            db = this.getReadableDatabase();
            @SuppressLint("Recycle") Cursor loCursor = db.rawQuery("SELECT * FROM User_Info_Master ORDER BY dSessionx DESC LIMIT 1", null);
    ...
    public void LogoutUser(){
        try{
            ...
            db.setTransactionSuccessful();
            Log.e(TAG, "User details has been deleted.");
        } catch (Exception e){
            e.printStackTrace();
        } finally {
            db.endTransaction();
